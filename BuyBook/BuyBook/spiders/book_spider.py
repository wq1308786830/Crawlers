...
class BookSpider(scrapy.Spider):
    ...
    params = {'keywords': '爱'}
    allow_domains = [
        'https://www.amazon.cn/s/ref=sr_st_relevancerank',
        'http://search.dangdang.com/',
        'https://search.jd.com/Search'
    ]
    amazon_url = allow_domains[0] + '?keywords=' + params['keywords']
    ...
    def parse(self, response):
        ...
        header_data = [['书名', 50], ['作者', 40], ['价格', 10], ['出版社', 40], ['时间', 30],
        ...
        if page == 'www.amazon.cn':
            ...
            max_page = response.xpath('//div[@id="pagn"]/span[@class="pagnDisabled"]/text()').extract_first()
            max_page = int(max_page) if max_page else 0
